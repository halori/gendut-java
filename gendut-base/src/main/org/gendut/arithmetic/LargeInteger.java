...
public class LargeInteger {
	 ...
	 */
	public static final LargeInteger ZERO = new LargeInteger(0, 0);

	/**
	 * The {@code BigInteger} constant 1.
	 */
	public static final LargeInteger ONE = new LargeInteger(1, 1);

	/**
	 * The {@code BigInteger} constant 10.
	 */
	public static final LargeInteger TEN = new LargeInteger(1, 10);

	/** The {@code BigInteger} constant -1. */
	static final LargeInteger MINUS_ONE = new LargeInteger(-1, 1);
	...
	static final LargeInteger[] SMALL_VALUES = { ZERO, ONE, new LargeInteger(1, 2),
			new LargeInteger(1, 3), new LargeInteger(1, 4), new LargeInteger(1, 5),
			new LargeInteger(1, 6), new LargeInteger(1, 7), new LargeInteger(1, 8),
			new LargeInteger(1, 9), TEN };

	static final LargeInteger[] TWO_POWS;

	static {
		TWO_POWS = new LargeInteger[32];
		for (int i = 0; i < TWO_POWS.length; i++) {
			TWO_POWS[i] = LargeInteger.valueOf(1L << i);
		}
	}
...
//	private int signum;
	 ...
	 *             if {@code numBits} < 0.
	 */
	public LargeInteger(int numBits, Random rnd) {
		if (numBits < 0) {
	 ...
	 *             if {@code bitLength} < 2.
	 */
	public LargeInteger(int bitLength, int certainty, Random rnd) {
		if (bitLength < 2) {
			// math.1C=bitLength < 2
			throw new ArithmeticException(Messages.getString("math.1C")); //$NON-NLS-1$
		}
		LargeInteger me = Primality.consBigInteger(bitLength, certainty, rnd);
		sign = me.sign;
	 ...
	 */
	public LargeInteger(int signum, byte[] magnitude) {
		if (magnitude == null) {
			throw new NullPointerException();
		}
			...
			for (byte element : magnitude) {
				if (element != 0) {
					...
					throw new NumberFormatException(
							Messages.getString("math.14")); //$NON-NLS-1$
				}
			}
		}
	 ...
	 */
	LargeInteger(int sign, int value) {
		this.sign = sign;
		numberLength = 1;
	 ...
	 */
	LargeInteger(int sign, int numberLength, int[] digits) {
		this.sign = sign;
	 ...
	 */
	LargeInteger(int sign, long val) {
	 ...
	 */
	LargeInteger(int signum, int digits[]) {
		if (digits.length == 0) {
			sign = 0;
			numberLength = 1;
	...
	public static LargeInteger valueOf(long val) {
		if (val < 0) {
			if (val != -1) {
				return new LargeInteger(-1, -val);
			}
			return MINUS_ONE;
		} else if (val <= 10) {
	...
	/** @see LargeInteger#BigInteger(String, int) */
	private static void setFromString(LargeInteger bi, String val, int radix) {
		int sign;
		int[] digits;
		int numberLength;
	...
	public LargeInteger abs() {
		return ((sign < 0) ? new LargeInteger(1, numberLength, digits) : this);
	}
	...
	public LargeInteger negate() {
		return ((sign == 0) ? this
				: new LargeInteger(-sign, numberLength, digits));
	}
	...
	public boolean equals(Object x) {
		...
		if (x instanceof LargeInteger) {
			LargeInteger x1 = (LargeInteger) x;
	...
	public LargeInteger gcd(LargeInteger val) {
		LargeInteger val1 = this.abs();
		LargeInteger val2 = val.abs();
		...
		if (((val1.numberLength == 1) || ((val1.numberLength == 2) && (val1.digits[1] > 0)))
			...
			return LargeInteger.valueOf(Division.gcdBinary(val1.longValue(),
					val2.longValue()));
		}
	 ...
	 * @see #remainder
	 */
	public LargeInteger[] divideAndRemainder(LargeInteger divisor) {
		...
		LargeInteger result0 = new LargeInteger(quotientSign, quotientLength,
				quotientDigits);
		LargeInteger result1 = new LargeInteger(thisSign, remainderLength,
				remainderDigits);
	...
	public LargeInteger divide(LargeInteger divisor) {
		...
		LargeInteger result = new LargeInteger(resSign, resLength, resDigits);
	 ...
	 *             if {@code divisor == 0}.
	 */
	public LargeInteger remainder(LargeInteger divisor) {
		if (divisor.sign == 0) {
			// math.17=BigInteger divide by zero
			throw new ArithmeticException(Messages.getString("math.17")); //$NON-NLS-1$
		}
		...
		LargeInteger result = new LargeInteger(sign, resLength, resDigits);
		result.cutOffLeadingZeroes();
		return result;
	}
	...
	public LargeInteger modInverse(LargeInteger m) {
		...
		// From now on: (m > 1)
		LargeInteger res = Division.modInverseMontgomery(abs().mod(m), m);
		if (res.sign == 0) {
			// math.19=BigInteger not invertible.
			throw new ArithmeticException(Messages.getString("math.19")); //$NON-NLS-1$
		}
	...
	public static LargeInteger probablePrime(int bitLength, Random rnd) {
		return new LargeInteger(bitLength, 100, rnd);
	}
	...
	LargeInteger copy() {
		...
		return new LargeInteger(sign, numberLength, copyDigits);
	}
	...
	static LargeInteger getPowerOfTwo(int exp) {
		...
		return new LargeInteger(1, intCount + 1, resDigits);
	}
}
