...
public class LargeDecimal {
	 ...
	 */
	public static final LargeDecimal ZERO = new LargeDecimal(0, 0);

	/**
	 * The constant one as a {@code BigDecimal}.
	 */
	public static final LargeDecimal ONE = new LargeDecimal(1, 0);

	/**
	 * The constant ten as a {@code BigDecimal}.
	 */
	public static final LargeDecimal TEN = new LargeDecimal(10, 0);
	 ...
	 * {@code ArithmeticException} for the case that rounding is necessary, i.e.
	 * for the case that the value cannot be represented exactly.
	 * 
	 ...
	 */
	private static final LargeInteger FIVE_POW[];

	/**
	 * An array with powers of ten that fit in the type <code>long</code> (
	 * <code>10^0,10^1,...,10^18</code>).
	 */
	private static final LargeInteger TEN_POW[];

	/**
	 * An array with powers of ten that fit in the type <code>long</code> (
	 * <code>10^0,10^1,...,10^18</code>).
	 */
	private static final long[] LONG_TEN_POW = new long[] { 1L, 10L, 100L,
			1000L, 10000L, 100000L, 1000000L, 10000000L, 100000000L,
			1000000000L, 10000000000L, 100000000000L, 1000000000000L,
			10000000000000L, 100000000000000L, 1000000000000000L,
			10000000000000000L, 100000000000000000L, 1000000000000000000L, };

	private static final long[] LONG_FIVE_POW = new long[] { 1L, 5L, 25L, 125L,
	 ...
	 */
	private static final LargeDecimal BI_SCALED_BY_ZERO[] = new LargeDecimal[BI_SCALED_BY_ZERO_LENGTH];

	/**
	 * An array with the zero number scaled by the first positive scales. (
	 * <code>0*10^0, 0*10^1, ..., 0*10^10</code>).
	 */
	private static final LargeDecimal ZERO_SCALED_BY[] = new LargeDecimal[11];
	...
	static {
		...
		for (; i < ZERO_SCALED_BY.length; i++) {
			BI_SCALED_BY_ZERO[i] = new LargeDecimal(i, 0);
			ZERO_SCALED_BY[i] = new LargeDecimal(0, i);
			CH_ZEROS[i] = '0';
		}
	...
	private LargeDecimal(long smallValue, int scale) {
		this.smallValue = smallValue;
		this.scale = scale;
		this.bitLength = bitLength(smallValue);
	}

	private LargeDecimal(int smallValue, int scale) {
		this.smallValue = smallValue;
		this.scale = scale;
		this.bitLength = bitLength(smallValue);
	}
	 ...
	 *             decimal.
	 */
	public LargeDecimal(char[] in, int offset, int len) {
		...
		if (bufLength < 19) {
			...
			setUnscaledValue(new LargeInteger(unscaledBuffer.toString()));
		}
	 ...
	 */
	public LargeDecimal(char[] in, int offset, int len, MathContext mc) {
		this(in, offset, len);
		inplaceRound(mc);
	}
	 ...
	 *             within the given precision without rounding.
	 */
	public LargeDecimal(char[] in, MathContext mc) {
		this(in, 0, in.length);
		inplaceRound(mc);
	}
	 ...
	 *             of a big decimal.
	 */
	public LargeDecimal(String val) {
	 ...
	 *             within the given precision without rounding.
	 */
	public LargeDecimal(String val, MathContext mc) {
		this(val.toCharArray(), 0, val.length());
		inplaceRound(mc);
	}
	 ...
	 * double. For example, {@code new BigDecimal(0.1)} is equal to
	 * {@code 0.1000000000000000055511151231257827021181583404541015625}. This
	 * happens as {@code 0.1} cannot be represented exactly in binary.
	 * <p>
	 ...
	 */
	public LargeDecimal(double val) {
		...
		if (scale < 0) {
			...
			if (bitLength < 64) {
				...
				intVal = LargeInteger.valueOf(mantisa).shiftLeft(-scale);
			}
			scale = 0;
		} else if (scale > 0) {
			// m * 2^e = (m * 5^(-e)) * 10^e
			if (scale < LONG_FIVE_POW.length
				...
				setUnscaledValue(Multiplication.multiplyByFivePow(
						LargeInteger.valueOf(mantisa), scale));
			}
		} else { // scale == 0
			smallValue = mantisa;
	 ...
	 *             within the given precision without rounding.
	 */
	public LargeDecimal(double val, MathContext mc) {
		this(val);
		inplaceRound(mc);
	}
	 ...
	 */
	public LargeDecimal(LargeInteger val) {
		this(val, 0);
	}
	 ...
	 *             within the given precision without rounding.
	 */
	public LargeDecimal(LargeInteger val, MathContext mc) {
		this(val);
		inplaceRound(mc);
	}
	 ...
	 */
	public LargeDecimal(LargeInteger unscaledVal, int scale) {
		if (unscaledVal == null) {
			throw new NullPointerException();
		}
		this.scale = scale;
	 ...
	 *             if {@code unscaledVal == null}.
	 */
	public LargeDecimal(LargeInteger unscaledVal, int scale, MathContext mc) {
	 ...
	 */
	public LargeDecimal(int val) {
		this(val, 0);
	}

	/**
	 * Constructs a new {@code BigDecimal} instance from the given int
	 * {@code val}. The scale of the result is {@code 0}. The result is rounded
	 ...
	 *             within the given precision without rounding.
	 */
	public LargeDecimal(int val, MathContext mc) {
		this(val, 0);
		inplaceRound(mc);
	}
	 ...
	 *             within the given precision without rounding.
	 */
	public LargeDecimal(long val, MathContext mc) {
		this(val);
		inplaceRound(mc);
	}
	 ...
	 *         10^(-{@code unscaledVal}).
	 */
	public static LargeDecimal valueOf(long unscaledVal, int scale) {
		if (scale == 0) {
		...
		return new LargeDecimal(unscaledVal, scale);
	}

	/**
	 * Returns a new {@code BigDecimal} instance whose value is equal to
	 * {@code unscaledVal}. The scale of the result is {@code 0}, and its
	 * unscaled value is {@code unscaledVal}.
	 * 
	 * @param unscaledVal
	 *            value to be converted to a {@code BigDecimal}.
	 * @return {@code BigDecimal} instance with the value {@code unscaledVal}.
	 */
	public static LargeDecimal valueOf(long unscaledVal) {
		if ((unscaledVal >= 0) && (unscaledVal < BI_SCALED_BY_ZERO_LENGTH)) {
			return BI_SCALED_BY_ZERO[(int) unscaledVal];
		}
		return new LargeDecimal(unscaledVal, 0);
	}

	/**
	 * Returns a new {@code BigDecimal} instance whose value is equal to
	 * {@code val}. The new decimal is constructed as if the
	 * {@code BigDecimal(String)} constructor is called with an argument which
	 * is equal to {@code Double.toString(val)}. For example,
	 * {@code valueOf("0.1")} is converted to (unscaled=1, scale=1), although
	 * the double {@code 0.1} cannot be represented exactly as a double value.
	 * In contrast to that, a new {@code BigDecimal(0.1)} instance has the value
	 * {@code 0.1000000000000000055511151231257827021181583404541015625} with an
	 * unscaled value
	 * {@code 1000000000000000055511151231257827021181583404541015625} and the
	 * scale {@code 55}.
	 * 
	 * @param val
	 ...
	 */
	public static LargeDecimal valueOf(double val) {
		if (Double.isInfinite(val) || Double.isNaN(val)) {
			// math.03=Infinity or NaN
			throw new NumberFormatException(Messages.getString("math.03")); //$NON-NLS-1$
		}
		return new LargeDecimal(Double.toString(val));
	}
	 ...
	 *             if {@code augend == null}.
	 */
	public LargeDecimal add(LargeDecimal augend) {
		...
		if (diffScale == 0) {
			...
			return new LargeDecimal(this.getUnscaledValue().add(
	...
	private static LargeDecimal addAndMult10(LargeDecimal thisValue,
			LargeDecimal augend, int diffScale) {
		if (diffScale < LONG_TEN_POW.length
				&& Math.max(thisValue.bitLength, augend.bitLength
						+ LONG_TEN_POW_BIT_LENGTH[diffScale]) + 1 < 64) {
			return valueOf(thisValue.smallValue + augend.smallValue
					* LONG_TEN_POW[diffScale], thisValue.scale);
		}
		return new LargeDecimal(thisValue.getUnscaledValue().add(
				Multiplication.multiplyByTenPow(augend.getUnscaledValue(),
						diffScale)), thisValue.scale);
	}
	 ...
	 */
	public LargeDecimal add(LargeDecimal augend, MathContext mc) {
		LargeDecimal larger; // operand with the largest unscaled value
		LargeDecimal smaller; // operand with the smallest unscaled value
		LargeInteger tempBI;
		...
		if (largerSignum == smaller.signum()) {
			tempBI = Multiplication.multiplyByPositiveInt(
					larger.getUnscaledValue(), 10).add(
					LargeInteger.valueOf(largerSignum));
		} else {
			tempBI = larger.getUnscaledValue().subtract(
					LargeInteger.valueOf(largerSignum));
			tempBI = Multiplication.multiplyByPositiveInt(tempBI, 10).add(
					LargeInteger.valueOf(largerSignum * 9));
		}
		// Rounding the improved adding
		larger = new LargeDecimal(tempBI, larger.scale + 1);
	 ...
	 */
	public LargeDecimal subtract(LargeDecimal subtrahend) {
		...
		if (diffScale == 0) {
			...
			return new LargeDecimal(this.getUnscaledValue().subtract(
			...
			if (diffScale < LONG_TEN_POW.length
					&& Math.max(this.bitLength, subtrahend.bitLength
							+ LONG_TEN_POW_BIT_LENGTH[diffScale]) + 1 < 64) {
				return valueOf(this.smallValue - subtrahend.smallValue
						* LONG_TEN_POW[diffScale], this.scale);
			}
			return new LargeDecimal(this.getUnscaledValue().subtract(
					Multiplication.multiplyByTenPow(
							subtrahend.getUnscaledValue(), diffScale)),
					this.scale);
		} else {// case s2 > s1 : [ u1 * 10 ^ (s2 - s1) - u2 , s2 ]
			diffScale = -diffScale;
			if (diffScale < LONG_TEN_POW.length
					&& Math.max(this.bitLength
							+ LONG_TEN_POW_BIT_LENGTH[diffScale],
							subtrahend.bitLength) + 1 < 64) {
				return valueOf(this.smallValue * LONG_TEN_POW[diffScale]
						- subtrahend.smallValue, subtrahend.scale);
			}
			return new LargeDecimal(Multiplication.multiplyByTenPow(
					this.getUnscaledValue(), diffScale).subtract(
	 ...
	 */
	public LargeDecimal subtract(LargeDecimal subtrahend, MathContext mc) {
		long diffScale = subtrahend.scale - (long) this.scale;
		int thisSignum;
		LargeDecimal leftOperand; // it will be only the left operand (this)
		LargeInteger tempBI;
		// Some operand is zero or the precision is infinity
		...
		if (subtrahend.aproxPrecision() < diffScale - 1) {
			...
			if (mc.getPrecision() < this.aproxPrecision()) {
				...
				if (thisSignum != subtrahend.signum()) {
					tempBI = Multiplication.multiplyByPositiveInt(
							this.getUnscaledValue(), 10).add(
							LargeInteger.valueOf(thisSignum));
				} else {
					tempBI = this.getUnscaledValue().subtract(
							LargeInteger.valueOf(thisSignum));
					tempBI = Multiplication.multiplyByPositiveInt(tempBI, 10)
							.add(LargeInteger.valueOf(thisSignum * 9));
				}
				// Rounding the improved subtracting
				leftOperand = new LargeDecimal(tempBI, this.scale + 1);
	 ...
	 */
	public LargeDecimal multiply(LargeDecimal multiplicand) {
		...
		/*
		 * Let be: this = [u1,s1] and multiplicand = [u2,s2] so: this x
		 * multiplicand = [ s1 * s2 , s1 + s2 ]
		 */
		if (this.bitLength + multiplicand.bitLength < 64) {
			return valueOf(this.smallValue * multiplicand.smallValue,
					toIntScale(newScale));
		}
		return new LargeDecimal(this.getUnscaledValue().multiply(
	 ...
	 */
	public LargeDecimal multiply(LargeDecimal multiplicand, MathContext mc) {
		LargeDecimal result = multiply(multiplicand);
	 ...
	 *             precision.
	 */
	public LargeDecimal divide(LargeDecimal divisor, int scale, int roundingMode) {
		...
		if (this.bitLength < 64 && divisor.bitLength < 64) {
			if (diffScale == 0) {
				...
				if (diffScale < LONG_TEN_POW.length
						&& divisor.bitLength
								+ LONG_TEN_POW_BIT_LENGTH[(int) diffScale] < 64) {
					return dividePrimitiveLongs(this.smallValue,
				...
				if (-diffScale < LONG_TEN_POW.length
						&& this.bitLength
								+ LONG_TEN_POW_BIT_LENGTH[(int) -diffScale] < 64) {
					return dividePrimitiveLongs(this.smallValue
							* LONG_TEN_POW[(int) -diffScale],
							divisor.smallValue, scale, roundingMode);
				}

			}
		}
		LargeInteger scaledDividend = this.getUnscaledValue();
		LargeInteger scaledDivisor = divisor.getUnscaledValue(); // for scaling of
																// 'u2'
		...
		return divideBigIntegers(scaledDividend, scaledDivisor, scale,
				roundingMode);
	}

	private static LargeDecimal divideBigIntegers(LargeInteger scaledDividend,
			LargeInteger scaledDivisor, int scale, int roundingMode) {

		LargeInteger[] quotAndRem = scaledDividend
				.divideAndRemainder(scaledDivisor); // quotient and remainder
		// If after division there is a remainder...
		LargeInteger quotient = quotAndRem[0];
		LargeInteger remainder = quotAndRem[1];
		if (remainder.signum() == 0) {
			return new LargeDecimal(quotient, scale);
		}
		int sign = scaledDividend.signum() * scaledDivisor.signum();
		int compRem; // 'compare to remainder'
		if (scaledDivisor.bitLength() < 63) { // 63 in order to avoid out of
												// long after <<1
			...
			compRem = remainder.abs().shiftLeftOneBit()
					.compareTo(scaledDivisor.abs());
			compRem = roundingBehavior(quotient.testBit(0) ? 1 : 0, sign
					* (5 + compRem), roundingMode);
		}
		if (compRem != 0) {
			...
			quotient = quotient.add(LargeInteger.valueOf(compRem));
			return new LargeDecimal(quotient, scale);
		}
	...
	private static LargeDecimal dividePrimitiveLongs(long scaledDividend,
			long scaledDivisor, int scale, int roundingMode) {
		...
		if (remainder != 0) {
			// Checking if: remainder * 2 >= scaledDivisor
			int compRem; // 'compare to remainder'
			compRem = longCompareTo(Math.abs(remainder) << 1,
					Math.abs(scaledDivisor));
			// To look if there is a carry
			quotient += roundingBehavior(((int) quotient) & 1, sign
					* (5 + compRem), roundingMode);
		}
		// Constructing the result with the appropriate unscaled value
	 ...
	 */
	public LargeDecimal divide(LargeDecimal divisor, int roundingMode) {
	...
	public LargeDecimal divide(LargeDecimal divisor) {
		LargeInteger p = this.getUnscaledValue();
		LargeInteger q = divisor.getUnscaledValue();
		LargeInteger gcd; // greatest common divisor between 'p' and 'q'
		LargeInteger quotAndRem[];
		...
		if (!q.abs().equals(LargeInteger.ONE)) {
		...
		return new LargeDecimal(p, newScale);
	}
	 ...
	 */
	public LargeDecimal divide(LargeDecimal divisor, MathContext mc) {
		/*
		 * Calculating how many zeros must be append to 'dividend' to obtain a
		 * quotient with at least 'mc.precision()' digits
		 */
		long traillingZeros = mc.getPrecision() + 2L + divisor.aproxPrecision()
				- aproxPrecision();
		long diffScale = (long) scale - divisor.scale;
		...
		LargeInteger integerQuot; // for temporal results
		LargeInteger quotAndRem[] = { getUnscaledValue() };
		...
		quotAndRem = quotAndRem[0].divideAndRemainder(divisor
				.getUnscaledValue());
		...
		if (quotAndRem[1].signum() != 0) {
			...
			integerQuot = integerQuot.multiply(LargeInteger.TEN).add(
					LargeInteger.valueOf(quotAndRem[0].signum() * (5 + compRem)));
			newScale++;
		} else {
		...
		// To perform rounding
		return new LargeDecimal(integerQuot, toIntScale(newScale), mc);
	}
	 ...
	 *             if {@code divisor == 0}.
	 */
	public LargeDecimal divideToIntegralValue(LargeDecimal divisor) {
		LargeInteger integralValue; // the integer of result
		LargeInteger powerOfTen; // some power of ten
		LargeInteger quotAndRem[] = { getUnscaledValue() };
		...
		if ((divisor.aproxPrecision() + newScale > this.aproxPrecision() + 1L)
			 ...
			 * part, the result must be zero with the appropriate scale
			 */
			integralValue = LargeInteger.ZERO;
		} else if (newScale == 0) {
		...
		return ((integralValue.signum() == 0) ? zeroScaledBy(newScale)
				: new LargeDecimal(integralValue, toIntScale(newScale)));
	}
	 ...
	 */
	public LargeDecimal divideToIntegralValue(LargeDecimal divisor, MathContext mc) {
		...
		LargeInteger quotAndRem[] = new LargeInteger[2];
		...
		// Let be: this = [u1,s1] and divisor = [u2,s2]
		if (quotPrecision <= 0) {
			quotAndRem[0] = LargeInteger.ZERO;
		} else if (diffScale == 0) {
			...
			// To chose 10^newScale to get a quotient with at least
			// 'mc.precision()' digits
			newScale = Math.min(diffScale,
					Math.max(mcPrecision - quotPrecision + 1, 0));
			// To calculate: (u1 / (u2 * 10^(s1-s2)) * 10^newScale
			quotAndRem[0] = quotAndRem[0].multiply(Multiplication
					.powerOf10(newScale));
		} else {// CASE s2 > s1:
			/*
			 ...
			 */
			long exp = Math.min(-diffScale,
					Math.max((long) mcPrecision - diffPrecision, 0));
			long compRemDiv;
			// Let be: (u1 * 10^exp) / u2 = [q,r]
			quotAndRem = this.getUnscaledValue()
			...
			if ((quotAndRem[1].signum() != 0) && (exp > 0)) {
				...
				compRemDiv = (new LargeDecimal(quotAndRem[1])).precision() + exp
						- divisor.precision();
				if (compRemDiv == 0) {
					// To calculate: (r * 10^exp2) / u2
					quotAndRem[1] = quotAndRem[1].multiply(
							Multiplication.powerOf10(exp)).divide(
							divisor.getUnscaledValue());
		...
		LargeInteger strippedBI = quotAndRem[0];
		LargeDecimal integralValue = new LargeDecimal(quotAndRem[0]);
	 ...
	 *             if {@code divisor == 0}.
	 */
	public LargeDecimal remainder(LargeDecimal divisor) {
	 ...
	 *             if {@code divisor == 0}.
	 * @throws ArithmeticException
	 *             if {@code mc.getPrecision() > 0} and the result of
	 *             {@code this.divideToIntegralValue(divisor, mc)} requires more
	 *             digits to be represented.
	 */
	public LargeDecimal remainder(LargeDecimal divisor, MathContext mc) {
	 ...
	 * @see #remainder
	 */
	public LargeDecimal[] divideAndRemainder(LargeDecimal divisor) {
		LargeDecimal quotAndRem[] = new LargeDecimal[2];
	 ...
	 * @see #remainder
	 */
	public LargeDecimal[] divideAndRemainder(LargeDecimal divisor, MathContext mc) {
		LargeDecimal quotAndRem[] = new LargeDecimal[2];
	...
	public LargeDecimal pow(int n) {
		...
		return ((isZero()) ? zeroScaledBy(newScale) : new LargeDecimal(
	 ...
	 *             if {@code n < 0} or {@code n > 999999999}.
	 */
	public LargeDecimal pow(int n, MathContext mc) {
		// The ANSI standard X3.274-1996 algorithm
		int m = Math.abs(n);
		int mcPrecision = mc.getPrecision();
		int elength = (int) Math.log10(m) + 1; // decimal digits in 'n'
		int oneBitMask; // mask of bits
		LargeDecimal accum; // the single accumulator
	 ...
	 * @return {@code abs(this)}
	 */
	public LargeDecimal abs(MathContext mc) {
		return round(mc).abs();
	}
	...
	public LargeDecimal negate() {
		...
		return new LargeDecimal(getUnscaledValue().negate(), scale);
	}
	 ...
	 * @return {@code -this}
	 */
	public LargeDecimal negate(MathContext mc) {
	 ...
	 */
	public LargeDecimal plus(MathContext mc) {
		return round(mc);
	}
	...
	public int precision() {
		...
		if (bitLength < 1024) {
			...
			/*
			 * To calculate the precision for large numbers Note that: 2
			 * ^(bitlength() - 1) <= intVal < 10 ^(precision())
			 */
			decimalDigits += (bitLength - 1) * LOG10_2;
			// If after division the number isn't zero, exists an aditional
			// digit
			if (getUnscaledValue().divide(
					Multiplication.powerOf10(decimalDigits)).signum() != 0) {
				decimalDigits++;
			}
		}
	 ...
	 */
	public LargeInteger unscaledValue() {
	 ...
	 *             precision.
	 */
	public LargeDecimal round(MathContext mc) {
		LargeDecimal thisBD = new LargeDecimal(getUnscaledValue(), scale);
	 ...
	 */
	public LargeDecimal setScale(int newScale, int roundingMode) {
		...
		if (diffScale > 0) {
			...
			if (diffScale < LONG_TEN_POW.length
				...
				return valueOf(this.smallValue * LONG_TEN_POW[(int) diffScale],
						newScale);
			}
			return new LargeDecimal(Multiplication.multiplyByTenPow(
					getUnscaledValue(), (int) diffScale), newScale);
		}
		// diffScale < 0
		// return [u,s] / [1,newScale] with the appropriate scale and rounding
		if (this.bitLength < 64 && -diffScale < LONG_TEN_POW.length) {
			return dividePrimitiveLongs(this.smallValue,
					LONG_TEN_POW[(int) -diffScale], newScale, roundingMode);
		}
	 ...
	 * {@code x.setScale(s).compareTo(x) == 0}.
	 * 
	 * @param newScale
	 *            scale of the result returned.
	 * @return a new {@code BigDecimal} instance with the specified scale.
	 ...
	 */
	public LargeDecimal setScale(int newScale) {
		return setScale(newScale, LargeDecimal.ROUND_UNNECESSARY);
	}
	 ...
	 */
	public LargeDecimal movePointLeft(int n) {
		return movePoint(scale + (long) n);
	}

	private LargeDecimal movePoint(long newScale) {
		if (isZero()) {
		...
		if (newScale >= 0) {
			...
			return new LargeDecimal(getUnscaledValue(), toIntScale(newScale));
		}
		...
		return new LargeDecimal(Multiplication.multiplyByTenPow(
				getUnscaledValue(), (int) -newScale), 0);
	}
	 ...
	 * @return {@code this * 10^n}.
	 */
	public LargeDecimal movePointRight(int n) {
		return movePoint(scale - (long) n);
	}

	/**
	 * Returns a new {@code BigDecimal} whose value is {@code this} 10^{@code n}
	 * . The scale of the result is {@code this.scale()} - {@code n}. The
	 * precision of the result is the precision of {@code this}.
	 * <p>
	 ...
	 * @return {@code this * 10^n}
	 */
	public LargeDecimal scaleByPowerOfTen(int n) {
		long newScale = scale - (long) n;
		if (bitLength < 64) {
	...
	/**
	 * Returns a new {@code BigDecimal} instance with the same value as
	 * {@code this} but with a unscaled value where the trailing zeros have been
	 ...
	 */
	public LargeDecimal stripTrailingZeros() {
		int i = 1; // 1 <= i <= 18
		int lastPow = TEN_POW.length - 1;
		long newScale = scale;

		if (isZero()) {
			return new LargeDecimal("0");
		}
		LargeInteger strippedBI = getUnscaledValue();
		LargeInteger[] quotAndRem;
		...
		return new LargeDecimal(strippedBI, toIntScale(newScale));
	}
	 ...
	 *             if {@code val == null}.
	 */
	public int compareTo(LargeDecimal val) {
		int thisSign = signum();
		int valueSign = val.signum();

		if (thisSign == valueSign) {
			if (this.scale == val.scale && this.bitLength < 64
					&& val.bitLength < 64) {
			...
			if (diffPrecision > diffScale + 1) {
				...
				return -thisSign;
			} else {// thisSign == val.signum() and diffPrecision is aprox.
					// diffScale
				LargeInteger thisUnscaled = this.getUnscaledValue();
				LargeInteger valUnscaled = val.getUnscaledValue();
				// If any of both precision is bigger, append zeros to the
				// shorter one
				if (diffScale < 0) {
					thisUnscaled = thisUnscaled.multiply(Multiplication
							.powerOf10(-diffScale));
				} else if (diffScale > 0) {
					valUnscaled = valUnscaled.multiply(Multiplication
							.powerOf10(diffScale));
				}
	...
	public boolean equals(Object x) {
		...
		if (x instanceof LargeDecimal) {
			LargeDecimal x1 = (LargeDecimal) x;
	 ...
	 *             if {@code val == null}.
	 */
	public LargeDecimal min(LargeDecimal val) {
	 ...
	 *             if {@code val == null}.
	 */
	public LargeDecimal max(LargeDecimal val) {
		return ((compareTo(val) >= 0) ? this : val);
	}
	...
	public String toPlainString() {
		...
		int delta = scale;
		// We take space for all digits, plus a possible decimal point, plus
		// 'scale'
	...
	public LargeInteger toLargeInteger() {
		if ((scale == 0) || (isZero())) {
			...
			return getUnscaledValue().multiply(
					Multiplication.powerOf10(-(long) scale));
		} else {// (scale > 0)
	 ...
	 */
	public LargeInteger toBigIntegerExact() {
		if ((scale == 0) || (isZero())) {
			return getUnscaledValue();
		} else if (scale < 0) {
			return getUnscaledValue().multiply(
			...
			LargeInteger[] integerAndFraction;
	...
	public double doubleValue() {
		...
		if (scale <= 0) {
			...
			LargeInteger quotAndRem[];
			LargeInteger powerOfTen = Multiplication.powerOf10(scale);
			int k = 100 - (int) powerOfTwo;
			int compRem;

			if (k > 0) {
				/*
				 * Computing (mantisa * 2^k) , where 'k' is a enough big power
				 * of '2' to can divide by 10^s
				 */
			...
			mantisa = quotAndRem[0].shiftLeft(2).add(
					LargeInteger.valueOf((compRem * (compRem + 3)) / 2 + 1));
			exponent -= 2;
		}
	...
	private void inplaceRound(MathContext mc) {
		...
		LargeInteger sizeOfFraction = Multiplication
				.powerOf10(discardedPrecision);
		LargeInteger[] integerAndFraction = getUnscaledValue()
		...
		if (integerAndFraction[1].signum() != 0) {
			...
			compRem = (integerAndFraction[1].abs().shiftLeftOneBit()
			...
			compRem = roundingBehavior(
					...
					mc.getRoundingMode());
			if (compRem != 0) {
				integerAndFraction[0] = integerAndFraction[0].add(LargeInteger
						.valueOf(compRem));
			}
			tempBD = new LargeDecimal(integerAndFraction[0]);
			// If after to add the increment the precision changed, we normalize
			// the size
			if (tempBD.precision() > mcPrecision) {
				integerAndFraction[0] = integerAndFraction[0]
						.divide(LargeInteger.TEN);
				newScale--;
			}
		}
	...
	private void smallRound(MathContext mc, int discardedPrecision) {
		...
		if (fraction != 0) {
			...
			integer += roundingBehavior(((int) integer) & 1,
					Long.signum(fraction) * (5 + compRem), mc.getRoundingMode());
			// If after to add the increment the precision changed, we normalize
			// the size
	...
	private static int roundingBehavior(int parityBit, int fraction,
		...
		switch (roundingMode) {
		case LargeDecimal.ROUND_UNNECESSARY:
			if (fraction != 0) {
				// math.08=Rounding necessary
				throw new ArithmeticException(Messages.getString("math.08")); //$NON-NLS-1$
			}
			break;
		case LargeDecimal.ROUND_UP:
			increment = Integer.signum(fraction);
			break;
		case LargeDecimal.ROUND_DOWN:
			break;
		case LargeDecimal.ROUND_CEILING:
			increment = Math.max(Integer.signum(fraction), 0);
			break;
		case LargeDecimal.ROUND_FLOOR:
			increment = Math.min(Integer.signum(fraction), 0);
			break;
		case LargeDecimal.ROUND_HALF_UP:
			if (Math.abs(fraction) >= 5) {
				increment = Integer.signum(fraction);
			}
			break;
		case LargeDecimal.ROUND_HALF_DOWN:
			if (Math.abs(fraction) > 5) {
				increment = Integer.signum(fraction);
			}
			break;
		case LargeDecimal.ROUND_HALF_EVEN:
	 ...
	 *         possible
	 * @throws ArithmeticException
	 *             when rounding is necessary or the number don't fit in the
	 *             primitive type
	 */
	private long valueExact(int bitLengthOfType) {
		LargeInteger bigInteger = toBigIntegerExact();
	 ...
	 * value will be {@code precision()} or {@code precision()-1} in the worst
	 * case.
	 * 
	 ...
	 * @throws ArithmeticException
	 *             when {@code scale} doesn't fit in {@code int} type
	 * @see #scale
	 */
	 ...
	 * {@code int}. if {@code longScale > Integer.MAX_VALUE} the scale will be
	 * {@code Integer.MAX_VALUE}; if {@code longScale < Integer.MIN_VALUE} the
	 * scale will be {@code Integer.MIN_VALUE}; otherwise {@code longScale} is
	 ...
	 * @see #scale
	 */
	private static LargeDecimal zeroScaledBy(long longScale) {
		if (longScale == (int) longScale) {
			return valueOf(0, (int) longScale);
		}
		if (longScale >= 0) {
			return new LargeDecimal(0, Integer.MAX_VALUE);
		}
		return new LargeDecimal(0, Integer.MIN_VALUE);
	}

	private LargeInteger getUnscaledValue() {
		if (intVal == null) {
			intVal = LargeInteger.valueOf(smallValue);
		}
		return intVal;
	}

	private void setUnscaledValue(LargeInteger unscaledValue) {
