 ...
 * arithmetic to {@link LargeInteger}. Some methods are provided in both mutable
 * and immutable way. There are several variants provided listed below:
 * 
 * <ul type="circle">
 * <li> <b>Division</b>
 * <ul type="circle">
 * <li>{@link LargeInteger} division and remainder by {@link LargeInteger}.</li>
 * <li>{@link LargeInteger} division and remainder by {@code int}.</li>
 * <li><i>gcd</i> between {@link LargeInteger} numbers.</li>
 * </ul>
 * </li>
 * <li> <b>Modular arithmetic </b>
 * <ul type="circle">
 * <li>Modular exponentiation between {@link LargeInteger} numbers.</li>
 * <li>Modular inverse of a {@link LargeInteger} numbers.</li>
 * </ul>
 * </li>
 *</ul>
 */
class Division {
     ...
     */
    static int remainder(LargeInteger dividend, int divisor) {
     ...
     */
    static LargeInteger[] divideAndRemainderByInteger(LargeInteger val,
        ...
        if (valLen == 1) {
            ...
            return new LargeInteger[] { LargeInteger.valueOf(quo),
                    LargeInteger.valueOf(rem) };
        }
        ...
        LargeInteger result0 = new LargeInteger(quotientSign, quotientLength,
                quotientDigits);
        LargeInteger result1 = new LargeInteger(valSign, 1, remainderDigits);
        result0.cutOffLeadingZeroes();
        result1.cutOffLeadingZeroes();
        return new LargeInteger[] { result0, result1 };
    }
     ...
     * @param op2
     *            must be greater than zero
     * @see LargeInteger#gcd(LargeInteger)
     * @return {@code GCD(op1, op2)}
     */
    static LargeInteger gcdBinary(LargeInteger op1, LargeInteger op2) {
        // PRE: (op1 > 0) and (op2 > 0)
        ...
        LargeInteger swap;
        // I want op2 > op1
        if (op1.compareTo(op2) == LargeInteger.GREATER) {
            swap = op1;
            op1 = op2;
            op2 = swap;
        } 
        ...
        do { // INV: op2 >= op1 && both are odd unless op1 = 0
            ...
            || ( ( op2.numberLength == 2 ) && ( op2.digits[1] > 0 ) )) {
                op2 = LargeInteger.valueOf(Division.gcdBinary(op1.longValue(),
                        op2.longValue()));
                break;
        }
            ...
            if (op2.numberLength > op1.numberLength * 1.2) {
                ...
                } while (op2.compareTo(op1) >= LargeInteger.EQUALS);
            }
            // now op1 >= op2
            swap = op2;
            op2 = op1;
    ...
    /**
     * Performs the same as {@link #gcdBinary(LargeInteger, LargeInteger)}, but
     ...
     * @param op2
     *            a positive number
     * @see #gcdBinary(LargeInteger, LargeInteger)
     ...
     * Inverse - Revised"
     */
    static LargeInteger modInverseMontgomery(LargeInteger a, LargeInteger p) {
        ...
        // PRE: a \in [1, p - 1]
        LargeInteger u, v, r, s;
        u = p.copy();  // make copy to use inplace method
        v = a.copy();
        int max = Math.max(v.numberLength, u.numberLength);
        r = new LargeInteger(1, 1, new int[max + 1]);
        s = new LargeInteger(1, 1, new int[max + 1]);
        s.digits[0] = 1;
        // s == 1 && v == 0
        ...
        while (v.signum() > 0) {
            ...
            while (u.compareTo(v) > LargeInteger.EQUALS) {
        ...
        if (r.compareTo(p) >= LargeInteger.EQUALS) {
        ...
        int n1 = calcN(p);
        if (k > m) {
            r = monPro(r, LargeInteger.ONE, p, n1);
            k = k - m;
        }
        
        r = monPro(r, LargeInteger.getPowerOfTwo(m - k), p, n1);
        return r;
    }
     ...
     */
    private static int calcN(LargeInteger a) {
     ...
     * @return bi == abs(2^exp)
     */
    private static boolean isPowerOfTwo(LargeInteger bi, int exp) {
     ...
     * @return
     */
    private static int howManyIterations(LargeInteger bi, int n) {
        int i = n - 1;
        if (bi.sign > 0) {
            while (!bi.testBit(i))
                i--;
    ...
    /**
     *
     * Based on "New Algorithm for Classical Modular Inverse" Rbert Lrencz.
     * LNCS 2523 (2002)
     *
     * @return a^(-1) mod m
     */
    static LargeInteger modInverseLorencz(LargeInteger a, LargeInteger modulo) {
        ...
        LargeInteger u = new LargeInteger(modulo.sign, modulo.numberLength,
                uDigits);
        LargeInteger v = new LargeInteger(a.sign, a.numberLength, vDigits);
        
        LargeInteger r = new LargeInteger(0, 1, new int[max + 1]); // BigInteger.ZERO;
        LargeInteger s = new LargeInteger(1, 1, new int[max + 1]);
        s.digits[0] = 1;
    ...
    static LargeInteger squareAndMultiply(LargeInteger x2, LargeInteger a2, LargeInteger exponent,LargeInteger modulus, int n2  ){
        LargeInteger res = x2;
     ...
     *                           BigInteger)
     */
    static LargeInteger slidingWindow(LargeInteger x2, LargeInteger a2, LargeInteger exponent,LargeInteger modulus, int n2){
        // fill odd low pows of a2
        LargeInteger pows[] = new LargeInteger[8];
        LargeInteger res = x2;
        int lowexp;
        LargeInteger x3;
        int acc3;
     ...
     * 
     * @see LargeInteger#modPow(LargeInteger, LargeInteger)
     * @see #monPro(LargeInteger, LargeInteger, LargeInteger, int)
     * @see #slidingWindow(LargeInteger, LargeInteger, LargeInteger, LargeInteger,
     *                      int)
     * @see #squareAndMultiply(LargeInteger, LargeInteger, LargeInteger, LargeInteger,
     *                      int)
     */
    static LargeInteger oddModPow(LargeInteger base, LargeInteger exponent,
            LargeInteger modulus) {
        // PRE: (base > 0), (exponent > 0), (modulus > 0) and (odd modulus)
        int k = (modulus.numberLength << 5); // r = 2^k
        // n-residue of base [base * r (mod modulus)]
        LargeInteger a2 = base.shiftLeft(k).mod(modulus);
        // n-residue of base [1 * r (mod modulus)]
        LargeInteger x2 = LargeInteger.getPowerOfTwo(k).mod(modulus);
        LargeInteger res;
        ...
        return monPro(res, LargeInteger.ONE, modulus, n2);
    }
     ...
     * @see LargeInteger#modPow(LargeInteger, LargeInteger)
     */
    static LargeInteger evenModPow(LargeInteger base, LargeInteger exponent,
            LargeInteger modulus) {
        ...
        LargeInteger q = modulus.shiftRight(j);

        // STEP 2: Compute x1 := base^exponent (mod q).
        LargeInteger x1 = oddModPow(base, exponent, q);

        // STEP 3: Compute x2 := base^exponent (mod 2^j).
        LargeInteger x2 = pow2ModPow(base, exponent, j);

        // STEP 4: Compute q^(-1) (mod 2^j) and y := (x2-x1) * q^(-1) (mod 2^j)
        LargeInteger qInv = modPow2Inverse(q, j);
        LargeInteger y = (x2.subtract(x1)).multiply(qInv);
        inplaceModPow2(y, j);
        if (y.sign < 0) {
            y = y.add(LargeInteger.getPowerOfTwo(j));
        }
     ...
     * @see LargeInteger#modPow(LargeInteger, LargeInteger)
     */
    static LargeInteger pow2ModPow(LargeInteger base, LargeInteger exponent, int j) {
        // PRE: (base > 0), (exponent > 0) and (j > 0)
        LargeInteger res = LargeInteger.ONE;
        LargeInteger e = exponent.copy();
        LargeInteger baseMod2toN = base.copy();
        LargeInteger res2;
        /*
    ...
    private static void monReduction(int[] res, LargeInteger modulus, int n2) {
     ...
     * @see #modPowOdd(LargeInteger, LargeInteger, LargeInteger)
     */
    static LargeInteger monPro(LargeInteger a, LargeInteger b, LargeInteger modulus, int n2) {
        int modulusLen = modulus.numberLength;
     ...
     */
    static LargeInteger finalSubtraction(int res[], LargeInteger modulus){
        ...
        LargeInteger result = new LargeInteger(1, modulusLen+1, res);
     ...
     */
    static LargeInteger modPow2Inverse(LargeInteger x, int n) {
        // PRE: (x > 0), (x is odd), and (n > 0)
        LargeInteger y = new LargeInteger(1, new int[1 << n]);
        y.numberLength = 1;
        y.digits[0] = 1;
        y.sign = 1;
     ...
     */
    static void inplaceModPow2(LargeInteger x, int n) {
        // PRE: (x > 0) and (n >= 0)
        int fd = n >> 5;
        int leadingZeros;
