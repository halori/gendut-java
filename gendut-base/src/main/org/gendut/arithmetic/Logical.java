...
class Logical {
    ...
    /** @see LargeInteger#not() */
    static LargeInteger not(LargeInteger val) {
        if (val.sign == 0) {
            return LargeInteger.MINUS_ONE;
        }
        if (val.equals(LargeInteger.MINUS_ONE)) {
        ...
        if (val.sign > 0) {
            ...
            if (val.digits[val.numberLength - 1] != -1) {
                ...
                if (i == val.numberLength) {
                    resDigits[i] = 1;
                    return new LargeInteger(-val.sign, i + 1, resDigits);
                }
            }
        ...
        return new LargeInteger(-val.sign, i, resDigits);
    }

    /** @see LargeInteger#and(LargeInteger) */
    static LargeInteger and(LargeInteger val, LargeInteger that) {
        if (that.sign == 0 || val.sign == 0) {
            return LargeInteger.ZERO;
        }
        if (that.equals(LargeInteger.MINUS_ONE)){
            return val;
        }
        if (val.equals(LargeInteger.MINUS_ONE)) {
            return that;
    ...
    static LargeInteger andPositive(LargeInteger val, LargeInteger that) {
        ...
        LargeInteger result = new LargeInteger(1, resLength, resDigits);
        result.cutOffLeadingZeroes();
        return result;
    }

    /** @return sign = positive.magnitude & magnitude = -negative.magnitude */
    static LargeInteger andDiffSigns(LargeInteger positive, LargeInteger negative) {
    ...
    static LargeInteger andNegative(LargeInteger longer, LargeInteger shorter) {
    ...
    /** @see LargeInteger#andNot(LargeInteger) */
    static LargeInteger andNot(LargeInteger val, LargeInteger that) {
        if (that.sign == 0 ) {
            return val;
        }
        if (val.sign == 0) {
    ...
    /** @return sign = 1, magnitude = val.magnitude & ~that.magnitude*/
    static LargeInteger andNotPositive(LargeInteger val, LargeInteger that) {
        // PRE: both arguments are positive
        ...
        LargeInteger result = new LargeInteger(1, val.numberLength, resDigits);
        result.cutOffLeadingZeroes();
        return result;
    }
    
    /** @return sign = 1, magnitude = positive.magnitude & ~(-negative.magnitude)*/
    static LargeInteger andNotPositiveNegative(LargeInteger positive, LargeInteger negative) {
    ...
    static LargeInteger andNotNegativePositive(LargeInteger negative, LargeInteger positive) {
    ...
    /** @return sign = 1, magnitude = -val.magnitude & ~(-that.magnitude)*/
    static LargeInteger andNotNegative(LargeInteger val, LargeInteger that) {
        // PRE: val < 0 && that < 0
    ...
    /** @see LargeInteger#or(LargeInteger) */
    static LargeInteger or(LargeInteger val, LargeInteger that) {
        if (that.equals(LargeInteger.MINUS_ONE) || val.equals(LargeInteger.MINUS_ONE)) {
            return LargeInteger.MINUS_ONE;
        }
        if (that.sign == 0) {
            return val;
    ...
    /** @return sign = 1, magnitude = longer.magnitude | shorter.magnitude*/
    static LargeInteger orPositive(LargeInteger longer, LargeInteger shorter) {
    ...
    /** @return sign = -1, magnitude = -(-val.magnitude | -that.magnitude) */
    static LargeInteger orNegative(LargeInteger val, LargeInteger that){
    ...
    /** @return sign = -1, magnitude = -(positive.magnitude | -negative.magnitude) */
    static LargeInteger orDiffSigns(LargeInteger positive, LargeInteger negative){
    ...
    /** @see LargeInteger#xor(LargeInteger) */
    static LargeInteger xor(LargeInteger val, LargeInteger that) {
        if (that.sign == 0) {
            return val;
        }
        if (val.sign == 0) {
    ...
    static LargeInteger xorPositive(LargeInteger longer, LargeInteger shorter) {
        // PRE: longer and shorter are positive;
        // PRE: longer has at least as many digits as shorter
        int resLength = longer.numberLength;
        int resDigits[] = new int[resLength];
    ...
    static LargeInteger xorNegative(LargeInteger val, LargeInteger that){
        // PRE: val and that are negative
    ...
    /** @return sign = 1, magnitude = -(positive.magnitude ^ -negative.magnitude)*/
    static LargeInteger xorDiffSigns(LargeInteger positive, LargeInteger negative){
