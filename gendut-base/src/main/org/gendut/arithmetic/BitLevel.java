 ...
 * {@link LargeInteger}. The operations are:
 * <ul type="circle">
 * <li>Left Shifting</li>
...
class BitLevel {
    ...
    /** @see LargeInteger#bitLength() */
    static int bitLength(LargeInteger val) {
        if (val.sign == 0) {
            return 0;
        }
    ...
    /** @see LargeInteger#bitCount() */
    static int bitCount(LargeInteger val) {
        int bCount = 0;
     ...
     */
    static boolean testBit(LargeInteger val, int n) {
    ...
    /** @see LargeInteger#shiftLeft(int) */
    static LargeInteger shiftLeft(LargeInteger source, int count) {
        ...
        LargeInteger result = new LargeInteger(source.sign, resLength, resDigits);
    ...
    static void inplaceShiftLeft(LargeInteger val, int count) {
     ...
     *      and the format is the same as {@link LargeInteger#digits}.
     * @param source the array of digits to shift left, in the same format as
     *      {@link LargeInteger#digits}.
    ...
    static LargeInteger shiftLeftOneBit(LargeInteger source) {
        int srcLen = source.numberLength;
        int resLen = srcLen + 1;
        int resDigits[] = new int[resLen];
        shiftLeftOneBit(resDigits, source.digits, srcLen);
        LargeInteger result = new LargeInteger(source.sign, resLen, resDigits);
        result.cutOffLeadingZeroes();
        return result;
    }

    /** @see LargeInteger#shiftRight(int) */
    static LargeInteger shiftRight(LargeInteger source, int count) {
        int intCount = count >> 5; // count of integers
        count &= 31; // count of remaining bits
        if (intCount >= source.numberLength) {
            return ((source.sign < 0) ? LargeInteger.MINUS_ONE : LargeInteger.ZERO);
        }
        int i;
        int resLength = source.numberLength - intCount;
     ...
     */
    static void inplaceShiftRight(LargeInteger val, int count) {
        int sign = val.signum();
     ...
     */
    static LargeInteger flipBit(LargeInteger val, int n){
