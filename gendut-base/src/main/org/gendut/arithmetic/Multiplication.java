...
/**
 * Static library that provides all multiplication of {@link LargeInteger} methods.
 */
class Multiplication {
     ...
     */
    static final LargeInteger[] bigTenPows = new LargeInteger[32];

    /**
     * An array with the first powers of five in {@code BigInteger} version.
     * ({@code 5^0,5^1,...,5^31})
     */
    static final LargeInteger bigFivePows[] = new LargeInteger[32];
    ...
    static {
        ...
        for (i = 0; i <= 18; i++) {
            bigFivePows[i] = LargeInteger.valueOf(fivePow);
            bigTenPows[i] = LargeInteger.valueOf(fivePow << i);
            fivePow *= 5;
        }
        for (; i < bigTenPows.length; i++) {
            bigFivePows[i] = bigFivePows[i - 1].multiply(bigFivePows[1]);
            bigTenPows[i] = bigTenPows[i - 1].multiply(LargeInteger.TEN);
        }
    }

    /**
     * Performs a multiplication of two BigInteger and hides the algorithm used.
     * @see LargeInteger#multiply(LargeInteger)
     */
    static LargeInteger multiply(LargeInteger x, LargeInteger y) {
     ...
     * @return {@code op1 * op2}
     * @see #multiply(LargeInteger, LargeInteger)
     */
    static LargeInteger karatsuba(LargeInteger op1, LargeInteger op2) {
        LargeInteger temp;
        ...
        LargeInteger upperOp1 = op1.shiftRight(ndiv2);
        LargeInteger upperOp2 = op2.shiftRight(ndiv2);
        LargeInteger lowerOp1 = op1.subtract(upperOp1.shiftLeft(ndiv2));
        LargeInteger lowerOp2 = op2.subtract(upperOp2.shiftLeft(ndiv2));

        LargeInteger upper = karatsuba(upperOp1, upperOp2);
        LargeInteger lower = karatsuba(lowerOp1, lowerOp2);
        LargeInteger middle = karatsuba( upperOp1.subtract(lowerOp1),
     ...
     */
    static LargeInteger multiplyPAP(LargeInteger a, LargeInteger b) {
        // PRE: a >= b
        ...
        if (resLength == 2) {
            ...
            return ((valueHi == 0)
            ? new LargeInteger(resSign, valueLo)
            : new LargeInteger(resSign, 2, new int[]{valueLo, valueHi}));
        }
     ...
     */
    static LargeInteger multiplyByPositiveInt(LargeInteger val, int factor) {
        ...
        if (aNumberLength == 1) {
            ...
            return ((resHi == 0)
            ? new LargeInteger(resSign, resLo)
            : new LargeInteger(resSign, 2, new int[]{resLo, resHi}));
        }
        // Common case
    ...
    static LargeInteger pow(LargeInteger base, int exponent) {
        // PRE: exp > 0
        LargeInteger res = LargeInteger.ONE;
        LargeInteger acc = base;
        ...
        for (; exponent > 1; exponent >>= 1) {
            ...
            if ( acc.numberLength == 1 ){
                ...
                acc = new LargeInteger(1, square(acc.digits, acc.numberLength, new int [acc.numberLength<<1]));
            }
        }
     ...
     */
    static LargeInteger multiplyByTenPow(LargeInteger val, long exp) {
        // PRE: exp >= 0
     ...
     */
    static LargeInteger powerOf10(long exp) {
        // PRE: exp >= 0
        int intExp = (int)exp;
        // "SMALL POWERS"
        if (exp < bigTenPows.length) {
            ...
            // To calculate:    10^exp
            return LargeInteger.TEN.pow(intExp);
        } else if (exp <= 1000) {
         ...
         */
        // To calculate:    5^exp
        LargeInteger powerOfFive = bigFivePows[1].pow(Integer.MAX_VALUE);
        LargeInteger res = powerOfFive;
     ...
     */
    static LargeInteger multiplyByFivePow(LargeInteger val, int exp) {
        // PRE: exp >= 0
